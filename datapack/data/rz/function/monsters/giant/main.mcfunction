# 巨人僵尸AI：半血后进入抛掷阶段、挥舞斧头攻击玩家并播放音效
...
execute on passengers at @s positioned ~ ~-3.425 ~ run data modify entity @s Rotation[0] set from entity @e[tag=rz,tag=giant,sort=nearest,limit=1] Rotation[0]
execute if entity @s[tag=!attacking,tag=!throwing] rotated ~ 0 positioned ^ ^ ^1 if entity @e[tag=player,distance=..2] run effect give @s minecraft:slowness 1 255 true
execute if entity @s[tag=!attacking,tag=!throwing] rotated ~ 0 positioned ^ ^ ^1 if entity @e[tag=player,distance=..2] run tag @s add attacking
...
execute if score @s rz.giant.animation matches 34 rotated ~ 0 positioned ^ ^ ^1 as @e[tag=player,distance=..2] run damage @s 15 minecraft:mob_attack by @e[tag=rz,tag=giant,sort=nearest,limit=1]
